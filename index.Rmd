---
title: "Refugees Statistics"
author: "Data, Information Management & Analysis (DIMA), MENA Bureau, Amman"
date: ""
output:
  html_document:
    number_sections: yes
...
UNHCR [Population Statistics Database](http://popstats.unhcr.org)  contains data about UNHCR's populations of concern from the year 1951 up to 2014 as well as their general composition by location of residence or origin, their status, their evolution over time. UNHCR's populations of concern status includes:
...
data <- read.csv(paste0(mainDir,"/data/unhcr_popstats_export_time_series_all_data.csv"), 
                 comment.char = "#",skip = 3, header = TRUE, as.is = TRUE, fileEncoding = "UTF8")
...
cow <- read.csv(paste0(mainDir,"/data/cow.txt"),skip = 28,  encoding = "UTF-8", sep = ";", comment.char = "#")
...
mena <- read.csv(paste0(mainDir,"/data/mena.csv"), encoding = "UTF-8", sep = ";", comment.char = "#")
...
gcircles <- gcIntermediate(arc[,c('longitude.x', 'latitude.x')], arc[,c('longitude.y', 'latitude.y')], 
                      n = 80, breakAtDateLine = FALSE, addStartEnd = TRUE, sp  = TRUE)
...
ggplot(data = popstats, aes(x = Year, y = refugees, alluvium =  Origin, stratum =  Origin)) +
  ggtitle("Syria has replaced Iraq as being the main country of Origin for Refugees in the Region", "the chart only displays origin with more than 10,000") + 
...
ggplot(data = popstats, aes(x = Year, y = refugees, alluvium = Asylum, stratum = Asylum)) +
  ...
  scale_fill_brewer(type = "qual", palette = "Set3") +  #,  guide = FALSE+
  ggrepel::geom_text_repel( aes(label = ifelse(Year == 2016, as.character(Asylum), NA),  stat = "stratum",
                                size = 4, direction = "y", nudge_x = 0, hjust = 0, segment.size = 0.2)) +
  theme(legend.position = "none") +
  theme( axis.text.x = element_text( hjust = 0),
         plot.title = element_text(hjust = 0.5, color = "#4e4d47"),
         plot.subtitle = element_text(hjust = 0.5, color = "#4e4d47", margin = margin(b = -0.1, t = -0.1, l = 2, unit = "cm"), debug = F),
...
ggplot(data = popstats, aes(x = Year, y = refugees, alluvium =  Origin, stratum =  Origin)) +
  ggtitle("Syria has replaced Iraq as being the main country of Origin in the Region", "the chart only displays origin with more than 10,000") + 
  geom_alluvium(aes(fill =  Origin),  alpha = .75, color = "white", decreasing = FALSE) +
  scale_y_continuous(labels = format_si()) +
  theme_bw() +
  labs( y = "# of Refugees", fill = "Hosting Countries")   + 
  #scale_colour_manual(values = paletteregion) +
  scale_fill_brewer(type = "qual", palette = "Set3") +  #,  guide = FALSE+
  ggrepel::geom_text_repel( aes(label = ifelse(Year == 2017, as.character( Origin), NA),  stat = "stratum", size = 4, direction = "y", nudge_x = 0, hjust = 0, segment.size = 0.2)) +
...
ggplot(data = popstats, aes(x = Year, y = refugees, alluvium = Asylum, stratum = Asylum)) +
  ggtitle("In the Gulf, mostly Kuwati, Saudi & Yemen have hosted refugees") + 
  geom_alluvium(aes(fill = Asylum),  alpha = .75, color = "white", decreasing = FALSE) +
  scale_y_continuous(labels = format_si()) +
  theme_bw() +
  labs( y = "# of Refugees", fill = "Hosting Countries")   + 
...
ggplot(data = popstats, aes(x = Year, y = refugees, alluvium =  Origin, stratum =  Origin)) +
  ggtitle("Most Refugees in the Gulf are Somalis", "the chart only displays origin with more than 5,000") + 
  geom_alluvium(aes(fill =  Origin),  alpha = .75, color = "white", decreasing = FALSE) +
  scale_y_continuous(labels = format_si()) +
  theme_bw() +
  labs( y = "# of Refugees", fill = "Hosting Countries")   + 
...
## Map for 2017 from Syria Situation Countries
